...
{
    public partial class AddChatConversationTable : Migration
    {
...
");
            ...
            // Batch rieng: cot conversationId vua them o tren can o batch truoc thi cac
            // cau lenh UPDATE ben duoi moi nhin thay duoc (SQL Server bien dich theo tung batch).
            migrationBuilder.Sql(@"
...
");
        ...
        {
            migrationBuilder.Sql(@"
IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE [name] = N'FK_Messages_ChatConversations_conversationId')
BEGIN
    ALTER TABLE [dbo].[Messages] DROP CONSTRAINT [FK_Messages_ChatConversations_conversationId];
END;

IF EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'IX_Messages_conversationId' AND [object_id] = OBJECT_ID(N'[dbo].[Messages]'))
    DROP INDEX [IX_Messages_conversationId] ON [dbo].[Messages];

IF COL_LENGTH(N'dbo.Messages', N'conversationId') IS NOT NULL
BEGIN
    ALTER TABLE [dbo].[Messages] DROP COLUMN [conversationId];
END;
